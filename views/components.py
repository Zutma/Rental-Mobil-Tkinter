...
class PrimaryButton(tk.Button):
    def __init__(self, parent, text, command, **kwargs):
        ...
        params.update(kwargs) # Biar user bisa override kalau mau
...
class ActionButton(tk.Button):
    ...
    def update_style(self, is_active):
        if is_active:
            self.config(state="normal", bg=self.active_color, fg=theme.COLOR_WHITE, highlightthickness=0)
        else:
            self.config(state="disabled", bg=theme.COLOR_WHITE, fg=theme.COLOR_DARK, 
                        disabledforeground=theme.COLOR_DARK, highlightbackground=self.active_color, highlightthickness=2)
