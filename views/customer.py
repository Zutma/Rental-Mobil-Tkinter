...
class CustomerView(tk.Frame):
    ...
    def do_add(self):
        ...
        form.set_mode("add")
        self.controller.show_view("CustomerFormView")

    def do_edit(self):
        selected = self.table.selection()
        if not selected: return
        idx = self.table.index(selected[0])
        cust_id = self._cust_ids[idx]
        rows = get_all_customers()
        data = None
        for r in rows:
            if r["id"] == cust_id:
    ...
    def do_delete(self):
        selected = self.table.selection()
        if not selected: return
        if not messagebox.askyesno("Konfirmasi", "Yakin ingin menghapus data pelanggan ini?"):
            return
        idx = self.table.index(selected[0])
        cust_id = self._cust_ids[idx]
        try:
    ...
    def do_save(self):
        ...
        try:
            if self._mode == "edit" and self._edit_id:
                ...
                add_customer(nik, nama, telp, alamat)
            messagebox.showinfo("Sukses", "Data pelanggan berhasil disimpan!")
            self.controller.show_view("CustomerView")
        except Exception as e:
